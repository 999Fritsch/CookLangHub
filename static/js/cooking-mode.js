/*
 * Cook mode: one card at a time, for the kitchen.
 *
 * Taken from CookCLI (static/js/cooking-mode.js).
 *
...
// ─── Start from the button, without an inline handler ──────────────
//
// CookCLI writes `onclick="startCookingMode()"`. The policy here forbids
// that, so the button carries `data-start-cooking` and is bound on load.
(function () {
    "use strict";

    function bind() {
        var button = document.querySelector("[data-start-cooking]");
        if (!button) return;

        // Cook mode needs the Recipe as data. Without it the button would
        // do nothing at all, so it goes away instead of failing quietly.
        if (!document.getElementById("cooking-mode-data")) {
            button.hidden = true;
            return;
        }
